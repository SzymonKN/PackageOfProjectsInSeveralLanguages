...
static Complex subtract(Complex c1,Complex c2){
	...
	c_sub.Re = c1.Re-c2.Re;
	c_sub.Im = c1.Im-c2.Im;
	return c_sub;
}

static Complex multiply(Complex c1,Complex c2){
	Complex c_mul = new Complex();
	c_mul.Re = c1.Re*c2.Re-c1.Im*c2.Im;
	c_mul.Im = c1.Im*c2.Re+c1.Re*c2.Im;

	return c_mul;
}

static Complex multiply(Complex c1,double x){
	Complex c_mul = new Complex();
	c_mul.Re =c1.Re*x;
	c_mul.Im=c1.Im*x;
...
static Complex divide(Complex c1,Complex c2){
	...
	c_div.Re =(c1.Re*c2.Re+c1.Im*c2.Im)/(c2.Re*c2.Re+c2.Im*c2.Im);
	c_div.Im=(c1.Im*c2.Re-c1.Re*c2.Im)/(c2.Re*c2.Re+c2.Im*c2.Im);
...
static Complex sqrt(double x){
	if(x>0)
		...
		return new Complex(0.0,Math.sqrt(x*-1));
}
